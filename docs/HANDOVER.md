...
결과 카드의 `김밥: O`(`dg_members.lunch`)와 그리드 🍙·요약(`dg_lunch`)은 **같은 김밥
탭**에서 나오지만 코드가 다르다. 카드는 **다가오는 회차 한 열**, 나머지는 모든 열.
그래서 새로 합류한 사람은 '카드 O · 신청 내역 없음' 이 구조적으로 났다 —
요약이 `getUpcomingLunch()` 로 다가오는 신청을 따로 읽어 함께 보여준다.
