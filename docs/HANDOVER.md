...
| 4 | 빈칸→X 를 언제 보낼지 | 조장 저장(`script.js:780`)과 관리자 `attBlanks()`(`admin.js:804`)가 **손대지 않은 빈칸까지** X 로 보낸다. 관리자 쪽은 조 선택이 '전체' 면 기수 전원이 범위다. 명단에 갓 올라온 사람이 지난 회차 결석으로 찍히는 경로 — 사용자 결정으로 **지금은 그대로 둔다**(시트를 비우면 되돌릴 수 있게 됐다) |
